...
> Install dependencies, configure env, expose the backend publicly, and run the RAG recipe locally. This recipe is **zero-key**: no LLM API key is needed. `CUSTOM_LLM_URL` must be a public URL because Agora cloud calls the `/llm` endpoint directly.
...
| `CUSTOM_LLM_URL`        |    ✅    | —                          | **Public** URL of `/llm/chat/completions`; Agora cloud calls it |
| `CUSTOM_LLM_API_KEY`    |    ✅    | `any-key-here`             | Forwarded as `Authorization: Bearer`; required by `CustomLLM`  |
...
# then add CUSTOM_LLM_URL=https://<your-tunnel>.ngrok-free.dev/llm/chat/completions
```
...
Agora cloud calls `/llm/chat/completions` directly. In local development, expose port 8000:
...
```bash
bun run dev              # backend (:8000, including /llm) + web (:3000) via concurrently
```

Open <http://localhost:3000> → **Start Conversation** → ask about refunds, business hours, shipping, or warranty. Backend API docs at <http://localhost:8000/docs>.

## Quick commands

```bash
bun run doctor           # shared prereqs (bun + node_modules); no creds needed
bun run doctor:local     # + .env.local + AGORA_APP_ID/CERTIFICATE + CUSTOM_LLM_URL

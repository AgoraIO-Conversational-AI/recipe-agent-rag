...
├── llm/      # RAG LLM endpoint (:8001) — OpenAI-compatible mock, no agora deps
...
Swap `CORPUS` and `retrieve()` in [`llm/src/custom_llm_server.py`](llm/src/custom_llm_server.py)
for a real vector store (e.g. ChromaDB, pgvector, Pinecone). The `run_agent_turn()`
